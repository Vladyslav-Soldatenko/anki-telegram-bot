...
class TestYourBotFunctionality(unittest.TestCase):
    ...
    async def test_handle_message_success(self, mock_logger, mock_append_to_csv, mock_get_word_definition):
        # Arrange
        ...
        mock_logger.info.assert_called_with("Definition for 'test': {'word': 'test', 'pronunciation': 'tɛst', 'definitions': [{'definition': 'A procedure intended to establish the quality, performance, or reliability of something.'}]}")        
        mock_append_to_csv.assert_called_once()
        mock_update.message.reply_text.assert_called_with("Message received!")

    @patch('helpers.get_word_definition', new_callable=AsyncMock)
    async def test_handle_message_no_definition(self, mock_logger, mock_get_word_definition):
        # Arrange
        mock_update = MagicMock(spec=Update)
        ...
        # Assert
        mock_logger.warning.assert_called_with("No definition found for 'unknown_word'.")
