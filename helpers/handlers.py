...
from .api import get_word_definition, WordDefinition
...
def prepare_csv_data(definition: WordDefinition) -> tuple[str, str]:
    ...
    back = "\n".join(
        [
            ...
            + (
                ...
                if "example" in defn and defn["example"]
                else ""
            )
...
def append_to_csv(front: str, back: str) -> None:
    ...
    csv_file_path = (
        os.path.join(CSV_PATH, "dictionary_definitions.csv") if CSV_PATH else ""
    )
